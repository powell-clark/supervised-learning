...
## Resolved project override

Source: <CLAUDE_PLUGIN_ROOT>/CONSCIOUSNESS/precepts/review-gates.yaml (plugin fallback — NOT this project's file)

Every row below is the EFFECTIVE gate this project resolves, computed through the same path isEntityAutoFlow takes to decide auto-close: the entity base gate from loadGateConfig, then the feature kano_overrides entry for the tier that feature's index row declares. The auto-close verdict on each row is the answer that path gives — requires: auto-approve, gate: off or gate: warn permit it, anything else gates. Where a row disagrees with the narrative above, the row is what the resolver does.

Every kano tier the loader honours is listed even when this project named only one. A project kano_overrides block MERGES onto the plugin kano map, so a tier the project did not name keeps its plugin default — except a tier the plugin map carries no entry for, which falls through to the entity-level feature gate above, wherever this project has moved it. A feature whose index row carries no kano tier resolves the **feature** row.

- **task**: requires: auto-approve, gate: off, min_agent_reviews: 0 — auto-close: yes (matches plugin default)
- **story**: requires: auto-approve, gate: off, min_agent_reviews: 0 — auto-close: yes (matches plugin default)
- **directive**: requires: auto-approve, gate: off, min_agent_reviews: 0 — auto-close: yes (matches plugin default)
- **feature**: requires: agent, gate: hard, min_agent_reviews: 1 — auto-close: no (plugin default: requires: human, gate: hard, min_agent_reviews: 1 — auto-close: no)
- **feature (must-have)**: requires: agent, gate: hard, min_agent_reviews: 1 — auto-close: no (plugin default: requires: human, gate: hard, min_agent_reviews: 1 — auto-close: no)
- **feature (performance)**: requires: agent, gate: hard, min_agent_reviews: 1 — auto-close: no (matches plugin default)
- **feature (delighter)**: requires: agent, gate: warn, min_agent_reviews: 1 — auto-close: yes (matches plugin default)
- **feature (nice-to-have)**: requires: agent, gate: warn, min_agent_reviews: 1 — auto-close: yes (matches plugin default)
- **feature (in-discussion)**: requires: agent, gate: hard, min_agent_reviews: 1 — auto-close: no (plugin default: requires: human, gate: hard, min_agent_reviews: 1 — auto-close: no)

Every min_agent_reviews figure above is (declared, not enforced — ADR review_gates--agent_tier_enforcement): nothing counts agent verdicts today.
